...
mono_image_open_from_data_with_name (char *data, guint32 data_len, gboolean need_copy, MonoImageOpenStatus *status, gboolean refonly, const char *name)
{
	...
	// +gusir for dll decrypt
	if( name != NULL &&
		strstr(name, "Assembly-CSharp.dll"))
	{
		char dnumber = 13;
		int dcount = 16;
		int dlength = data_len < dcount ? data_len : dcount;
		int i = 0;
		for(i=0; i < dlength; ++i)
		{
			data[i] ^= dnumber;
...
void mono_image_self_lib_decrypt(void)
{
	const char* key = mono_image_get_self_lib_key();
	g_message("so key is : %s", key);
    ...
    lib_addr = mono_image_get_self_lib_address(); 
    g_message("libaddr is : 0x%08x", lib_addr);
    ...
    g_message("text_addr is : 0x%08x", text_addr);
    ...
    g_message("mem privilege change success!");
 	...
 	// show all decrypt content
 	unsigned char* content = (unsigned char*)text_addr;
 	char* dcontent = (char*)malloc(nblock*2+1);
    memset(dcontent, 0, sizeof(char)*(nblock*2+1));
    for(i=0; i<nblock; ++i)
    {
    ...
    unsigned char* c1 = (char*)malloc(nblock+1);
    unsigned char* c2 = (char*)malloc(nblock+1);
    memset(c1, 0, sizeof(char)*(nblock+1));
    memset(c2, 0, sizeof(char)*(nblock+1));
    memcpy(c1, dcontent, sizeof(char)*(nblock));
    memcpy(c2, &dcontent[nblock], sizeof(char)*(nblock));
    g_message("%s", c1);
    g_message("%s", c2);

    // decrypt
    tea_decrypt(key, (unsigned char*)text_addr, nblock);
    /*for(i = 0; i < nblock; i++)
	{
...
unsigned long mono_image_get_self_lib_address(void)
{
    ...
    while(fgets(buffer, sizeof(buffer), fp)){
    	g_message("lib info line: %s", buffer);
...
const char* mono_image_get_self_lib_key()
{
	return "d2a460df08a4fb7a558f635b540d90cb";
}

const char* mono_image_get_dll_key()
{
	return "dcff57c9a964f83fbf81cc75ec2e413a";
}
